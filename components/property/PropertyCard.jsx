...
import { RiMapPin2Line, RiTimeLine, RiAuctionLine, RiHomeLine, RiRulerLine } from 'react-icons/ri'
...
export default function PropertyCard({ property, className = '' }) {
  ...
  return (
    <div className={`group block bg-white rounded-2xl border border-gray-200 overflow-hidden card-hover ${className}`}>
      {/* Image */}
      <div className="relative h-48 bg-gray-100 overflow-hidden">
        ...
        {/* Badges */}
        <span className={`absolute top-3 left-3 px-2.5 py-1 text-xs font-semibold rounded-xl ${trust.badgeClass}`}>
          {trust.label}
        </span>
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-1.5">
          ...
          {property.fields?.sft && (
            <div className="flex items-center gap-1 text-xs text-gray-400 flex-shrink-0">
              <RiRulerLine className="text-xs" />{property.fields.sft} sq.ft
            </div>
          )}
        </div>

        <p className="text-sm font-semibold text-gray-800 mb-1.5 line-clamp-1">{property.title}</p>
          ...
          <div className="flex items-center gap-3 mb-3 text-xs text-gray-500">
            {property.fields?.bhk && (
              <div className="flex items-center gap-1">
                <RiHomeLine className="text-gray-400 text-xs" />{property.fields.bhk} BHK
              </div>
            )}
            {property.fields?.sqYards && (
              <div className="flex items-center gap-1">
                <RiRulerLine className="text-gray-400 text-xs" />{property.fields.sqYards} Sq.Yd
              </div>
            )}
          </div>
        )}
        ...
        {property.tags?.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-3">
            {property.tags.slice(0, 3).map((tag) => (
              <span key={tag} className="px-2 py-0.5 bg-gray-100 text-gray-600 text-xs rounded-lg font-medium">{tag}</span>
            ))}
          </div>
        )}
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-100">
          <div className="flex items-center gap-1 text-xs text-gray-400">
            <RiTimeLine className="text-xs" />{timeAgo(property.createdAt)}
          </div>
          <Link
