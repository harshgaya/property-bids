...
import { RiShieldCheckLine, RiCameraLine, RiFileShieldLine, RiMapPin2Line, RiAddLine, RiArrowRightLine, RiStarFill } from 'react-icons/ri'
...
const floatingCards = [
  { title: '3 BHK Villa',      location: 'Jubilee Hills, Hyd', price: '₹85 L',    trust: 'Legal', delay: 'anim-float', bg: 'bg-green-50' },
  { title: '2 BHK Apartment',  location: 'Hinjewadi, Pune',    price: '₹42 L',    trust: 'Pre-Verified', delay: '', bg: 'bg-white' },
  { title: 'Residential Plot', location: 'Devanahalli, Blr',   price: '₹28 L',    trust: 'Pre-Verified', delay: '', bg: 'bg-white' },
]
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            <div className="anim-fade-up inline-flex items-center gap-2 px-4 py-2 bg-white/10 border border-white/20 rounded-full mb-8 backdrop-blur-sm">
              ...
              <span className="text-xs font-semibold text-white/90 tracking-widest uppercase">Verified · Transparent · Trusted</span>
            </div>
            ...
            <p className="anim-fade-up d-200 text-lg text-white/70 mb-8 max-w-lg leading-relaxed">
              India's first trust-first marketplace. Every listing GPS-verified with live photos. Serious buyers pay ₹99 to bid — no time wasters.
            </p>
            ...
            <div className="anim-fade-up d-400 flex flex-wrap gap-4 mb-12">
              <Link href="/post-property" className="inline-flex items-center gap-2 px-7 py-4 bg-green-500 text-white text-sm font-bold rounded-2xl hover:bg-green-400 transition-all shadow-lg shadow-green-900/30 hover:shadow-green-500/30 hover:-translate-y-0.5">
                Post Property <RiAddLine className="text-base" />
              </Link>
              <Link href="/map" className="inline-flex items-center gap-2 px-7 py-4 bg-white/10 border border-white/20 text-white text-sm font-bold rounded-2xl hover:bg-white/20 transition-all backdrop-blur-sm">
            ...
            <div className="relative">
              ...
              <div className="relative anim-float bg-gray-900 rounded-[3rem] p-2.5 shadow-2xl border border-white/10 z-10">
                <div className="bg-white rounded-[2.5rem] overflow-hidden w-64">
                  ...
                  <div className="relative h-56 bg-green-50 overflow-hidden">
                    ...
                    {/* Blocks */}
                    {[[5,5,23,28],[35,5,27,28],[5,42,22,22],[35,42,27,22],[68,5,27,22],[68,35,27,28]].map(([l,t,w,h],i)=>(
                      <div key={i} className="absolute bg-green-100 rounded-sm" style={{left:`${l}%`,top:`${t}%`,width:`${w}%`,height:`${h}%`}} />
                    ))}
                    {/* Pins */}
                    {[{top:'22%',left:'42%'},{top:'18%',left:'72%'},{top:'52%',left:'18%'},{top:'55%',left:'55%'}].map((pos,i)=>(
                  ...
                  <div className="p-3.5">
                    <div className="relative h-28 bg-gradient-to-br from-gray-700 to-gray-500 rounded-2xl mb-3 overflow-hidden">
                      ...
                      <span className="absolute top-2 right-2 px-2 py-0.5 bg-green-500 text-white text-xs font-bold rounded-lg">VERIFIED</span>
                      <div className="absolute bottom-2 left-3">
                        <div className="flex gap-0.5">
                          {[...Array(5)].map((_,i) => <RiStarFill key={i} className="text-yellow-400 text-xs" />)}
                        </div>
                      </div>
                    </div>
                    <p className="text-xs font-bold text-gray-900">3 BHK Independent House</p>
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-base font-extrabold text-gray-900">₹1.25 Cr</span>
                      <span className="text-xs px-2 py-1 bg-green-50 text-green-700 font-semibold rounded-lg">Price-Filtered</span>
                    </div>
                    <p className="text-xs text-gray-400 mt-1.5">1,850 sq.ft · 3 Beds · 3 Baths</p>
                  </div>
                </div>
              </div>
              ...
              <div className="absolute top-8 -right-6 lg:-right-14 flex flex-col gap-2.5 z-20">
                {floatingCards.map((c, i) => (
                  <div key={i} className={`w-48 ${c.bg} rounded-2xl shadow-xl border border-gray-100 p-3 flex gap-2.5 items-center card-hover`} style={{animationDelay:`${i*200}ms`}}>
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-1">
                        <span className="text-xs font-extrabold text-gray-900">{c.price}</span>
                        <span className="text-xs px-1.5 py-0.5 bg-green-50 text-green-700 rounded-lg font-medium">{c.trust}</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="absolute -bottom-4 -left-4 bg-white rounded-2xl shadow-xl border border-gray-100 px-4 py-3 flex items-center gap-3 z-20">
                ...
                <div>
                  <p className="text-xs font-bold text-gray-900">Zero Fake Listings</p>
                  <p className="text-xs text-gray-400">GPS + Live photo verified</p>
                </div>
              </div>
            </div>
          </div>
