  ...
  try {
    ...
    const incomingBids =
      listingIds.length > 0
        ? await bidsCol
            .aggregate([
              { $match: { propertyId: { $in: listingIds } } },
              { $sort: { createdAt: -1 } },
              {
                $lookup: {
                  from: "properties",
              ...
              },
              {
                $addFields: {
                  ...
                  propertyPrice: { $arrayElemAt: ["$property.priceLabel", 0] },
                },
              },
              { $project: { property: 0 } },
            ])
    ...
    const placedBids = await bidsCol
      .aggregate([
        ...
        { $sort: { createdAt: -1 } },
        {
          $lookup: {
            ...
            as: "property",
            pipeline: [
              { $project: { title: 1, priceLabel: 1, "address.city": 1 } },
            ],
          },
        },
        {
          $addFields: {
            propertyTitle: { $arrayElemAt: ["$property.title", 0] },
            propertyPrice: { $arrayElemAt: ["$property.priceLabel", 0] },
            city: { $arrayElemAt: ["$property.address.city", 0] },
          },
        },
        { $project: { property: 0 } },
        {
